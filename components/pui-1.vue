<template>
	...
	<view class="gui-form-body" 
				@tap="showKeyboard1(item.ref)">
				<text class="gui-text gui-block-text">{{item.score ==  '' ? '点此处' : item.score}}</text>
	</view>
	<pui-number-keyboard :items="selectType"
		:ref="item.ref" 
		@confirm="confirm1"></pui-number-keyboard>				
			<view class="gui-lr-margin" >
				<view class="gui-select-list-content gui-flex gui-column gui-align-items-center">
					...
					<!-- <view class="gui-form-body">
							...
							v-model="item.score" name="score" placeholder="请评分" />
						</view> -->
					<text class="gui-list-title-text gui-color-orange1">分数：{{item.score}}</text>	
				</view>
			</view>
...
export default{
	...
	methods : {
		// 第1个示例
				showKeyboard1 : function(val){
					console.log(val,'ref')
						this.$refs.val.open();
						},
						confirm1      : function (val,item) {
							item.score = val;
						},
