<template>
	<view class="gui-flex gui-rows gui-wrap">
		...
		v-for="(item, index) in imgLists" :key="index">
			<image :src="item.Filename" :data-imgurl="item.Filename" 
    ...
    methods : {
		initImgs     : function(){
			...
			for(let i = 0; i < this.items.length; i++){
				imgs.push({Filename:this.items[i]});
			}
			this.imgLists = imgs;
		},
        addImg       : function(){
                ...
                success    : (res) => {
					...
					for(let i = 0; i < res.tempFilePaths.length; i++){
						...
						const uploadTask = uni.uploadFile({
							url : that.domain+'upload/path/moments',
							...
							success: function (uploadFileRes) {
								...
								if(data.errno === 0){
									...
									// that.imgLists.push({Filename:data.msg,WebID:'6',CategoryID:'1',Title:'精彩图片',Description:'xx'});
									that.imgLists.push({Filename:data.msg});
                    ...
                    this.$emit('change', this.imgLists,this.uploadUrl);
                },
				complete   : function(){}
            });
        },
