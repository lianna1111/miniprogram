			...
			:style="{height:mainHeight+'px'}">
				...
				<view class="gui-padding" v-if="step == 1">
					...
					<view class="gui-form-body">
						...
						v-model="formData.username" name="username" placeholder="请输入教师用户名" />
					</view>
				</view>
				<view class="gui-form-item gui-border-b">
				...
				<view class="gui-form-item gui-border-b">
					<text class="gui-form-label"><text class="gui-color-red">*</text>昵称</text>
					<view class="gui-form-body">
						<input type="text" class="gui-form-input" 
				...
				<view class="gui-form-item gui-border-b">
					...
					<view class="gui-form-body">
						<input type="text" class="gui-form-input" 
						v-model="formData.CampusID" name="CampusID" placeholder="请输入所属校区编号" />
					</view>
				</view>
				<view class="gui-form-item gui-border-b">
...
export default {
	...
	onLoad: function() {
	this.formData.Uuid=uni.getStorageSync('uid')
		// 01. 获取页面主体高度
		 	...
		 	uni.chooseImage({
		 		...
		 		success: (chooseImageRes) => {
		 			...
		 			const uploadTask = uni.uploadFile({
		 				url : 'https://api.yiwenyw.com/upload/path/user',
		...
		selectImg1(){
			...
			uni.chooseImage({
				...
				success: (chooseImageRes) => {
					...
					//图片上传
					const uploadTask = uni.uploadFile({
						url : this.domain+'upload/path/user',
						filePath: tempFilePaths[0],
						name: 'file',
						success: function (uploadFileRes) {
							var data = JSON.parse(uploadFileRes.data);
			...
			uni.request({
			    ...
			    success: (res) => {
					if(res.data.errno==0){
						...
						// url: '../home/index?utype=0'//传参：0为机构
						});	
					}
					else{
						uni.showToast({
