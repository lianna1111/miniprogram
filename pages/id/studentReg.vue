		...
		<view slot="gBody" class="gui-flex1 gui-flex gui-columns">
			...
			:style="{height:mainHeight+'px'}">
				...
				<view class="gui-form-item gui-border-b">
					<text class="gui-form-label"><text class="gui-color-red">*</text>用户名</text>
					<view class="gui-form-body">
						<input type="text" class="gui-form-input" 
						v-model="formData.username" name="UserName" placeholder="请输入学生用户名(4-16位,无特殊符号)" />
					</view>
				</view>
				<view class="gui-form-item gui-border-b">
					...
					<view class="gui-form-body">
						<input type="text" class="gui-form-input" 
						v-model="formData.NickName" name="NickName" placeholder="请输入学生昵称" />
					</view>
				</view>
				<view class="gui-form-item gui-border-b">
				
					<text class="gui-form-label">真实姓名</text>
					<view class="gui-form-body">
						<input type="text" class="gui-form-input" 
						v-model="formData.RealName" name="RealName" placeholder="请输入学生真实姓名" />
					</view>
				</view>
				<view class="gui-form-item gui-border-b">
					<text class="gui-form-label"><text class="gui-color-red">*</text>学号</text>
					<view class="gui-form-body">
						<input type="text" class="gui-form-input" 
						v-model="formData.StudentCode" name="StudentCode" placeholder="请输入学生学号" />
					</view>
				</view>
				<view class="gui-form-item gui-border-b">
					<text class="gui-form-label"><text class="gui-color-red">*</text>学生照片</text>
					...
					<!-- <view class="gui-form-body">
						<input type="text" class="gui-form-input" 
						v-model="formData.Headimg" name="Headimg" placeholder="请输入学生头像" />
					</view> -->
				</view>
				 <view class="gui-form-item gui-border-b">
					<text class="gui-form-label">手机号码</text>
					<view class="gui-form-body">
						<input type="text" class="gui-form-input" 
						v-model="formData.Mobile" name="Mobile" placeholder="请输入学生手机号码" />
					</view>
				</view> 
				<view class="gui-form-item gui-border-b">
				...
				<view class="gui-form-item gui-border-b">
					...
					<view class="gui-form-body">
						...
						v-model="formData.Age" name="Age" placeholder="请输入学生年龄" />
					</view>
				</view>
				...
				<view class="gui-form-item gui-border-b">
					...
					<view class="gui-form-body">
						<input type="number" class="gui-form-input" 
						v-model="formData.Height" name="Height" placeholder="请输入学生身高(单位:mm)" />
					</view>
				</view>
				<view class="gui-form-item gui-border-b">
					...
					<view class="gui-form-body">
						<input type="number" class="gui-form-input" 
						v-model="formData.Weight" name="Weight" placeholder="请输入学生体重(单位:g)" />
					</view>
				</view>
				...
				<view class="gui-padding" v-if="step == 3">
						...
						<view class="gui-margin-top-large">
							<text class="gui-h6 gui-color-gray gui-bold">监护人</text>
						</view>
						...
						<view class="gui-form-item gui-border-b">
							...
							<view class="gui-form-body">
								<input type="text" class="gui-form-input"
								v-model="formData.GuardianName" name="GuardianName" placeholder="请输入监护人姓名" />
							</view>
						</view>
						<view class="gui-form-item gui-border-b">
							...
							<view class="gui-form-body">
								<input type="number" class="gui-form-input"
								v-model="formData.GuardianMobile" name="GuardianMobile" placeholder="请输入监护人手机号" />
							</view>
						</view>
						<view class="gui-form-item gui-border-b">
							<text class="gui-form-label"><text class="gui-color-red">*</text>类型</text>
							<view class="gui-form-body">
								
								<picker mode="selector" :range="Guardian"  range-key="CategoryName" @change="pickerChangeType">
									<view class="gui-flex gui-rows gui-nowrap gui-space-between gui-align-items-center">
										<text class="gui-text">{{Guardian[GuardianIndex].CategoryName}}</text>
										<text class="gui-form-icon gui-icons gui-text-center gui-color-gray">&#xe603;</text>
									</view>
								</picker>
							</view>				
...
export default {
	data() {
		return {
			...
			step: 1,// 步骤
			Gender : ['请选择学生性别', '女', '男'],//piker：性别
			GenderIndex : 0,//pickerindex
			Blood : ['请选择学生血型', 'A', 'B', 'AB', 'O'],//piker：血型
			BloodIndex : 0,//pickerindex
			Cale : ['请选择公历/农历', '公历', '农历'],//piker：性别
			CaleIndex : 0,//pickerindex
			Guardian : ['请选择监护人类型', '爸爸', '妈妈', '爷爷', '奶奶', '其他'],//piker：监护人类型
			GuardianIndex : 0,//pickerindex
			// 表单数据记录
			formData: {
				type:'api',
				GradeID:'74',
	...
	onLoad: function() {
		...
		uni.request({
		    url: this.domain+'guardian', //提交表单
			method:'post',
		    data: '',
		      header:{token:'alaskdflasdf','content-type': 'application/x-www-form-urlencoded'},
		    success: (res) => {
			this.Guardian=res.data.msg
			this.Guardian.unshift({
			ID:0,
			CategoryName:'请选择监护人类型'
			})
		    },
			fail() {
			}
	...
	methods: {
		...
		pickerChangeType : function (e) {
			...
			this.formData.GuardianType = this.Guardian[this.GuardianIndex];
		},					
		// 省市区
		 	...
		 	uni.chooseImage({
		 		...
		 		success: (chooseImageRes) => {
		 			...
		 			const uploadTask = uni.uploadFile({
		 				url : that.domain+'upload/path/user/student',
		...
		stepto : function(index){
			...
			switch(index){
				...
				break;
				case 2:
				   if(!this.formData.username||!this.formData.Headimg||!this.formData.Gender||!this.formData.Age
				...
				break;
				case 3:
				  if(!this.formData.Idcard||!this.formData.IsLunar||this.formData.IdcardFont == 'https://img-cdn-tc.dcloud.net.cn/uploads/article/20210107/e53bf48607dcf795ab78e419dc5cf432.png' ||
		...
		submit:function () {
			...
			// 请根据实际需求进行数据校验及提交
			if(!this.formData.GuardianName||!this.formData.GuardianMobile) 
			{uni.showToast({title:"必填项请填写完整", icon:"none"});
			return}
			else{
				//性别
				...
				else {uni.showToast({title:"请选择学生性别", icon:"none"});}
				//血型
				...
				//类型
				if(this.formData.GuardianType){this.formData.GuardianType=this.formData.GuardianType.ID}
				else{this.formData.GuardianType='0'}
				//公历农历
			...
			uni.request({
			    ...
			    success: (res) => {
					if(res.data.errno==0){
						...
						console.log(res,"res")
						uni.setStorageSync('isStudent',true);//???
