			...
			:style="{height:mainHeight+'px'}">
				...
				<view class="gui-form-item gui-border-b">
					...
					<view class="gui-form-body">
						...
						v-model="formData.StudentName" name="StudentName" placeholder="请输入学生真实姓名" />
					</view>
				</view>
				<view class="gui-form-item gui-border-b">
				...
				<view class="gui-form-item gui-border-b">
					<text class="gui-form-label"><text class="gui-color-red">*</text>学生寸照</text>
	...
	methods: {
		 	...
		 	uni.chooseImage({
		 		...
		 		success: (chooseImageRes) => {
		 			...
		 			const uploadTask = uni.uploadFile({
		 				url : that.domain+'upload/path/student',
		...
		stepto : function(index){
			...
			switch(index){
				...
				break;
				case 2:
				   if(!this.formData.StudentName||!this.formData.Headimg||!this.formData.Gender
				...
				break;
				case 3:
				  if(!this.formData.Idcard||!this.formData.IsLunar||!this.formData.Age
				   ) 
				  {uni.showToast({title:"必填项请填写完整", icon:"none"});
				  return}
				// 第3步数据验证这里编写
		...
		submit:function () {
			...
			// 请根据实际需求进行数据校验及提交
			if(!this.formData.GuardianName||!this.formData.GuardianMobile||!this.formData.GuardianType) 
			{uni.showToast({title:"必填项请填写完整", icon:"none"});
			return}
			else{
				//身份证
				if(this.formData.IdcardFont == 'https://img-cdn-tc.dcloud.net.cn/uploads/article/20210107/e53bf48607dcf795ab78e419dc5cf432.png') {
					this.formData.IdcardFont = ''; this.formData.IdcardBack =''
				}
				//性别
			...
			uni.request({
			    url: this.domain+'campus/add_student', //提交表单
				method:'post',
			    data: this.formData,
			      header:{token:'alaskdflasdf','content-type': 'application/x-www-form-urlencoded'},
			    success: (res) => {
				...
				uni.redirectTo({
			     ...
			     url: './student?campusID='+this.campusID+'&title='+this.title
				});
				}
			    },
				fail() {
